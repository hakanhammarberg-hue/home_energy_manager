...
PLACEHOLDER_VALUES = {"your_db_token_here"}
...
def is_influxdb_configured() -> bool:
    ...
    if config["token"] in PLACEHOLDER_VALUES:
        return False
...
def get_influxdb_config():
    ...
    1. Environment variables (HA_DB_URL, HA_DB_BUCKET, HA_DB_ORG, HA_DB_TOKEN)
    ...
    Returns:
        dict: Configuration with url, bucket, org, and token keys
    ...
    org = os.getenv("HA_DB_ORG")
    token = os.getenv("HA_DB_TOKEN")

    # If all environment variables are set, use them
    if url and org and token and bucket:
    ...
    return {
        ...
        "org": influxdb.get("org", ""),
        "token": influxdb.get("token", ""),
...
def test_influxdb_connection() -> dict:
    ...
    if not url or not org or not token or not bucket:
...
"""
    ...
    try:
        ...
        if response.status_code != 200:
            status_messages = {
                401: "Wrong or expired InfluxDB API token",
        ...
        return {
            ...
            "message": (
                ...
                f"Current bucket: '{bucket}', org: '{org}'. "
                f"Verify the bucket name matches an InfluxDB 2.x bucket exactly "
                f"(no retention-policy suffix, e.g. 'homeassistant' not "
                f"'homeassistant/autogen') and that the token has read access to it."
            ),
        }
...
def get_sensor_data(sensors_list, start_time=None, stop_time=None) -> dict:
    ...
    if not url or not org or not token:
        _LOGGER.error(
            "InfluxDB configuration is incomplete. URL: %s, Org: %s",
            url,
            org,
        )
        return {"status": "error", "message": "Incomplete InfluxDB configuration"}
